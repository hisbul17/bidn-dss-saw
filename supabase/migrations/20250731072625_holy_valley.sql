...
-- =====================================================
-- Version: 2.1 - Fixed for MySQL Local Server
...
ON DUPLICATE KEY UPDATE 
    ...
    rank_in_department = VALUES(rank_in_department),
    rank_overall = VALUES(rank_overall),
    is_best_in_department = VALUES(is_best_in_department),
    is_best_overall = VALUES(is_best_overall);

-- =====================================================
-- STORED PROCEDURES AND FUNCTIONS (FIXED)
-- =====================================================

-- Drop existing procedures if they exist
DROP PROCEDURE IF EXISTS RecalculateEmployeeScores;
DROP PROCEDURE IF EXISTS UpdateRankings;

-- Delimiter change for stored procedures
DELIMITER //

-- Procedure to recalculate employee scores (FIXED - No transaction control)
...
-- Procedure to update rankings (FIXED - No transaction control)
...
CREATE INDEX IF NOT EXISTS idx_evaluations_employee_period_criteria ON evaluations(employee_id, period_id, criteria_id);
CREATE INDEX IF NOT EXISTS idx_employee_scores_period_weighted ON employee_scores(period_id, weighted_score DESC);
